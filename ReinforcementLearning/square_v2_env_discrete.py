...
register(
    ...
    entry_point='square_v2_env_discrete:SquareEnv'
)
...
class SquareEnv(gymnasium.Env):
    ...
    def step(self, action):
        ...
        if action == SHRINK_LEFT:
            self.view[0] = (self.view[0]-1)*0.95+1
        elif action == SHRINK_TOP:
            self.view[1] = (self.view[1]-1)*0.95+1
        elif action == SHRINK_RIGHT:
            self.view[2] = self.view[2]*0.95
        elif action == SHRINK_BOTTOM:
            self.view[3] = self.view[3]*0.95
        ...
        #print(self.view, (x1,y1,x2,y2), self.bb.get_bb_corners())
